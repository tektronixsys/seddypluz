...
import { Sparkles, ChevronLeft, ChevronRight, Star, ArrowRight, Eye } from "lucide-react";
...
export function HeroAccordionCarousel() {
  ...
  return (
    ...
    >
      ...
      {/* Main Container */}
      <div className="relative overflow-hidden rounded-[2.5rem] bg-white/40 backdrop-blur-xl border border-plum/15 p-2.5 sm:p-3.5 shadow-[0_30px_100px_rgba(82,58,77,0.16)]">
        {/* Horizontal Expanding / Contracting Flex Stack */}
        <div className="flex h-[420px] sm:h-[480px] md:h-[520px] gap-2 sm:gap-2.5 w-full items-stretch">
          {heroSlides.map((slide, idx) => {
            ...
            return (
              <div
                ...
                className={`relative overflow-hidden rounded-[1.8rem] transition-all duration-700 cubic-bezier(0.25, 1, 0.5, 1) cursor-pointer group select-none ${
                  isActive
                    ? "flex-[3.6] md:flex-[4] shadow-xl"
                    : "flex-[0.9] sm:flex-[0.8] hover:flex-[1.1] opacity-75 hover:opacity-100"
                }`}
                role="button"
                tabIndex={0}
              ...
              >
                ...
                {isActive ? (
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-plum/90 via-plum/30 to-transparent flex flex-col justify-between p-5 sm:p-6 text-white animate-in fade-in duration-500">
                    {/* Top Bar: Vol Badge & Number */}
                    <div className="flex items-center justify-between">
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-white/20 backdrop-blur-md px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[#FAF9F5] border border-white/25">
                        <Sparkles className="h-3 w-3 text-amber-300" />
                        <span>{slide.vol}</span>
                      </span>

                      <span className="font-display text-2xl sm:text-3xl italic text-amber-300/90 font-semibold drop-shadow-sm">
                        {slide.num}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1.5 sm:space-y-2">
                      <span className="inline-block rounded-md bg-amber-400/20 px-2 py-0.5 text-[9px] sm:text-[10px] font-bold uppercase tracking-wider text-amber-300 backdrop-blur-sm border border-amber-300/30">
                        {slide.tag}
                      </span>

                      <h3 className="font-display text-2xl sm:text-3xl md:text-4xl text-white leading-tight italic">
                        {slide.title}
                      </h3>
                  ...
                  <div className="absolute inset-0 bg-plum/60 hover:bg-plum/45 backdrop-blur-[2px] transition-colors duration-300 flex flex-col justify-between items-center py-5 px-1 sm:px-2 text-[#FAF9F5]">
                    {/* Top Index */}
                    <span className="font-display text-sm sm:text-base italic text-amber-300/80 font-semibold">
                      {slide.num}
                    </span>
                    ...
                    <div className="flex-1 flex items-center justify-center my-2">
                      <span
                        className="text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.22em] text-[#FAF9F5]/90 whitespace-nowrap"
                        style={{
        ...
        <div className="mt-3 px-2 flex items-center justify-between gap-3 text-plum">
          ...
          <div className="flex items-center gap-1.5 flex-1 max-w-[200px]">
            {heroSlides.map((_, i) => (
              <button
                ...
                className="h-1.5 flex-1 rounded-full overflow-hidden bg-plum/15 hover:bg-plum/30 transition-colors cursor-pointer"
              >
                <div
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full border border-plum/15 bg-white/80 text-plum hover:bg-plum hover:text-[#FAF9F5] active:scale-95 transition-all cursor-pointer shadow-xs"
            >
      ...
      <div
        className="absolute -left-4 sm:-left-6 -top-4 hidden sm:block z-20 animate-float-up pointer-events-none"
